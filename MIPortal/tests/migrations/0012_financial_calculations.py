# Generated by Django 3.1.7 on 2021-03-18 05:39

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('Investors', '0002_entity_entitybusinessroimapping_entityinvestorrormapping_investor_invoice_transaction_wallet'),
        ('tests', '0011_gst_customers_gst_suppliers'),
    ]

    operations = [
        migrations.CreateModel(
            name='Financial_calculations',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('sales_one', models.FloatField(blank=True, null=True)),
                ('ebitdta_one', models.FloatField(blank=True, null=True)),
                ('ebidta_per_m_one', models.FloatField(blank=True, null=True)),
                ('business_id', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='Investors.business')),
            ],
        ),
    ]
